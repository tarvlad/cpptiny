...
class RBTree {
    struct Node {
        ...
        ~Node();
    } __attribute__(( aligned(sizeof(size_t) * 2) ));
...
template <std::three_way_comparable K, typename V>
bool RBTree<K, V>::is_red(Node *node) {
    if (node != nullptr) {
        return node->red == 1;
    }
    return false;
...
template <std::three_way_comparable K, typename V>
RBTree<K, V>::Node *RBTree<K, V>::single_rot(Node *root, uint8_t dir) {
    assert(root != nullptr);
    assert(dir == 0 || dir == 1);
...
template <std::three_way_comparable K, typename V>
RBTree<K, V>::Node *RBTree<K, V>::double_rot(Node *root, uint8_t dir) {
    assert(root != nullptr); 
    assert(dir == 0 || dir == 1);
...
template <std::three_way_comparable K, typename V>
size_t RBTree<K, V>::black_height(Node *root) {
